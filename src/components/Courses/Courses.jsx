        ...
        return data.filter((item) =>
            keys.some((key) => {
                return item[key]
                    .toString()
                    .toLowerCase()
                    .includes(searchCourse.toString().toLowerCase())
            })
        )
    }
    const coursesCount = [
        ...
        <>
            ...
            <div className='wrapper'>
                <div className='courses'>
                    <h6 className='courses__subtitle subtitle'>
                        Enjoy your studying!
                    </h6>
                    ...
                    <div className='courses__filter'>
                        {coursesCount.map(({ sup, title }, idx) => {
                            return (
                                <span
                                    key={idx + title}
                                    onClick={() => courseFilter(title, courses)}
                                >
                                    {title} <sup>{sup}</sup>{' '}
                                </span>
                            )
                    ...
                    <div className='courses__cards main-courses'>
                        {inputSearch(filtered).map((item, idx) => {
                            ...
                            return (
                                <div
                                    key={idx + alt}
                                    className='courses__card course'
                                >
                                    <Link to={`/courses/${idx + 1}`}>
                                        ...
                                        <div className='courses__card-content'>
                                            <span
                                                className={`courses__spec spec ${specColor}`}
                                            >
                                                {spec}
                                            </span>
